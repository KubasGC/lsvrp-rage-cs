iopctrl = function () {
    ...
    iopctrl.ledarray = function () {
        ...
        ledarray.value = function (x) {
            ...
            _ledarrayUpdate.each(function (d, i) {
                if ((!inv && r > d) || (inv && r < (d + _h))) this._led.on(true)
            });
            _ledarrayUpdate.reverse().each(function (d, i) {
                if ((!inv && r <= d) || (inv && r >= (d + _h))) this._led.on(false)
            });
            return ledarray;
        };
    ...
    iopctrl.slider = function () {
        ...
        var _range, _invert, _vertical, _lastEvent, _currentValue, _currentPoint, _slide, _deltaX = 0,
            _deltaY = 0,
            _lastPos = {};
        var _onValueChanged;
        var _indicator;
    ...
    var iopctrl_axisDefaultOrient = "out",
        iopctrl_axisOrients = { in: 1,
            out: 1
        };
    ...
    function iopctrl_axisSubdivide(scale, ticks, m) {
        subticks = [];
        if (m && ticks.length > 1) {
            var extent = iopctrl_extent(scale.domain()),
                subticks, i = -1,
                n = ticks.length,
                d = (ticks[1] - ticks[0]) / ++m,
                j, v;
